...
function buildChart(playerId, playerTeamId) {
  ...
  d3.json(`/shotchart/${playerId}/${playerTeamId}`).then(individual => {
    ...
    let totalDistance = (
      shotDistance.reduce((a, b) => a + b, 0)
    )

    let avgDistance = (totalDistance / shotDistance.length).toFixed(2)
    ...
    let layout = {
      ...
      annotations: [
        ...
        },
        {
          ...
          xref: 'x',
          yref: 'y',
          text: `FG M: ${madeBaskets}/${shotMade.length}`,
          showarrow: false,
          ax: 0,
          ay: -40
        },
        {
          ...
          xref: 'x',
          yref: 'y',
          text: `FG Pct: ${((madeBaskets / (shotMade.length)).toFixed(4) * 100).toFixed(2)}%`,
          showarrow: false,
          ax: 0,
          ay: -40
        },
        {
          ...
          xref: 'x',
          yref: 'y',
          text: `Avg. Distance: ${avgDistance} ft.`,
          showarrow: false,
          ax: 0,
          ay: -40
        }
  ...
  d3.json(`/playerdashboardextended/${playerId}`).then(response => {
    ...
    let dataTable = [{
      ...
      header: {
        ...
        fill: { color: '#4A494A' },
        font: { family: "Helvetica Neue", size: 16, color: "white" }
      },
      cells: {
        values: valuesTable,
        align: ["left", "center"],
        ...
        font: { family: "Helvetica Neue", size: 13, color: ["#4A494A"] }
      }
    }]
